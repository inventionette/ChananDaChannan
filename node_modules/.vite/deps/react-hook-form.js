...
function useFormState(props) {
	...
	_name.current = name;
	import_react.useEffect(() => control._subscribe({
		name: _name.current,
...
function useWatch(props) {
	...
	const [value, updateValue] = import_react.useState(control._getWatch(name, defaultValue));
...
function getDirtyFieldsFromDefaultValues(data, formValues, dirtyFieldsFromValues) {
	const isParentNodeArray = Array.isArray(data);
	if (isObject(data) || isParentNodeArray) for (const key in data) if (Array.isArray(data[key]) || isObject(data[key]) && !objectHasFunction(data[key])) {
		if (isUndefined(formValues) || isPrimitive(dirtyFieldsFromValues[key])) dirtyFieldsFromValues[key] = Array.isArray(data[key]) ? markFieldsDirty(data[key], []) : { ...markFieldsDirty(data[key]) };
...
function useFieldArray(props) {
	...
	import_react.useEffect(() => control._subjects.array.subscribe({ next: ({ values, name: fieldArrayName }) => {
...
function useForm(props = {}) {
	...
	import_react.useLayoutEffect(() => control._subscribe({
