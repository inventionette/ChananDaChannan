...
export function RootLayout() {
  ...
  const [mobileScholarshipOpen, setMobileScholarshipOpen] = useState(false);

  // Close mobile menu when route changes
  useEffect(() => {
    setMobileMenuOpen(false);
    setMobileBiographyOpen(false);
    setMobileScholarshipOpen(false);
  }, [location.pathname]);
  ...
  const scholarshipLinks = [
    { to: "/scholarship", labelKey: "scholarship.aboutProgram" },
    { to: "/apply", labelKey: "nav.apply" },
  ];
        ...
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex flex-1 items-center justify-center gap-8 lg:gap-10 xl:gap-12">
              ...
              {/* Scholarship Dropdown - NEW */}
              <div
                className="relative"
                onMouseEnter={() => setScholarshipDropdownOpen(true)}
              ...
              >
                ...
                >
                  ...
                  <ChevronDown className="w-3 h-3 transition-transform duration-300 group-hover:text-gray-600" style={{ transform: scholarshipDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)' }} />
                </button>
                {(location.pathname === "/scholarship" || location.pathname === "/apply") && (
                  <motion.div
                    layoutId="activeNav"
                    className="absolute -bottom-1 left-0 right-0 h-0.5 bg-black"
                    transition={{
              ...
              {/* Regular Nav Links - Gallery and About */}
              {navLinks.map((link) => {
                // Skip scholarship as it's now a dropdown
                if (link.to === "/scholarship") return null;
                return (
                  <Link
                    key={link.to}
                    to={link.to}
            ...
            <>
              ...
              >
                <div className="container mx-auto px-4 py-6 flex flex-col items-center gap-4">
                  ...
                  {/* Scholarship with Submenu - NEW for mobile */}
                  <motion.div
                    initial={{ opacity: 0, y: -6 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <button
                      onClick={() => setMobileScholarshipOpen(!mobileScholarshipOpen)}
                      className="flex items-center justify-center gap-2 w-full text-lg uppercase tracking-wider font-medium hover:text-gray-600 transition-colors py-2"
                      style={{ fontFamily: "'Work Sans', sans-serif" }}
                    >
                      {t("nav.scholarship")}
                      <ChevronDown
                        className="w-4 h-4 transition-transform duration-300"
                        style={{ transform: mobileScholarshipOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}
                      />
                    </button>

                    {/* Scholarship Submenu */}
                    <AnimatePresence>
                      {mobileScholarshipOpen && (
                        <motion.div
                          initial={{ opacity: 0, height: 0 }}
                          animate={{ opacity: 1, height: "auto" }}
                          exit={{ opacity: 0, height: 0 }}
